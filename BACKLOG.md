...
Choosing a home is a separate decision from recognizing a capability: shared behavior belongs to the package that naturally owns it, and nothing lands in `B44.Common` by default. A primitive that turns up independently in a second repository is an ownership-review trigger, recorded with both call sites in the relevant work item, not an automatic extraction.
