...
- A primitive that turns up independently in a second repository is an extraction *candidate*, not an extraction. Record it in `B44.Common`'s backlog with both call sites and decide ownership there. Nothing automates this: whether two near-identical functions are the same concept, or the same formula serving different intents, is a design judgement.
...
one commit. `B44.Unity`'s CLAUDE.md carries the matching checklist.
